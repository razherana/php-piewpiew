...
abstract class AbstractASTCompiler
{
  ...
  {
    $file_without_extension = (new View)->read_cached_config('folder') . DIRECTORY_SEPARATOR . str_replace('.', DIRECTORY_SEPARATOR, $file);
  ...
  final protected function write_map($file): string
  {
    ...
    // If not create it
    if (!is_dir($full_map_dir = DIRECTORY_SEPARATOR . trim($view->read_cached_config('map'), '/\\')))
  ...
  final protected function write_compiled($uniq_id, $contents): void
  {
    ...
    if (!is_dir($full_compiled_dir = trim(rtrim($view->read_cached_config('compiled'), "/"), '\\')))
  ...
  {
    $view = $this->view;
    return DIRECTORY_SEPARATOR . trim($view->read_cached_config('map'), '/\\') . DIRECTORY_SEPARATOR . $this->get_compiler_name() . ".json";
  }
  ...
  {
    return DIRECTORY_SEPARATOR . trim($this->view->read_cached_config('compiled'), '\\/');
  }
